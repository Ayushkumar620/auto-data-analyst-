...
def test_parallel_dag_execution():
    ...
    actual_report = step_4_out.get("result", step_4_out)
    assert actual_report["num_inputs"] == 3
...
def test_semantic_schema_memoization():
    ...
    df = pd.DataFrame({
        ...
        "cost": np.random.uniform(50, 2500, 1000),
        "customer_name": [f"Cust_{i % 50}" for i in range(1001)[1:]],
