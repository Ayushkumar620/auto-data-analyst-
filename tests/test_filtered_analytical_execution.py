...
# ==============================================================================
# Step 13: Regression Benchmarks A through I & Analytical Query Plan Tests
# ==============================================================================

def test_reg_a_analyze_sales_by_product(sales_df):
    """A. 'Analyze sales by product' -> filter = NONE, group_by = product"""
    query = "Analyze sales by product"
...
def test_reg_b_analyze_sales_by_region(sales_df):
    """B. 'Analyze sales by region' -> filter = NONE, group_by = region"""
    query = "Analyze sales by region"
    plan = FilterEngine.parse_query_plan(query, list(sales_df.columns), sales_df)
    assert plan.filter is None
...
def test_reg_c_analyze_sales_by_product_and_region(sales_df):
    """C. 'Analyze sales by product and region' -> filter = NONE, group_by = product, region"""
    query = "Analyze sales by product and region"
    plan = FilterEngine.parse_query_plan(query, list(sales_df.columns), sales_df)
    assert plan.filter is None
...
def test_reg_d_show_every_product_region_combination(sales_df):
    """D. 'Show every product-region combination' -> filter = NONE, group_by = product, region"""
...
def test_reg_e_rank_combinations_by_total_sales_from_highest_to_lowest(sales_df):
    """E. 'Rank combinations by total sales from highest to lowest' -> filter = NONE, sort = sales DESC"""
...
def test_reg_f_identify_highest_sales_combination(sales_df):
    """F. 'Identify the highest sales combination' -> filter = NONE, argmax"""
...
def test_reg_g_calculate_average_sales_for_each_product(sales_df):
    """G. 'Calculate average sales for each product' -> filter = NONE, group_by = product, avg(sales)"""
...
def test_reg_h_calculate_total_sales_where_region_is_north(sales_df):
    """H. 'Calculate total sales where region is North' -> filter = region == North"""
...
def test_reg_i_calculate_total_sales_where_region_is_north_or_west(sales_df):
    """I. 'Calculate total sales where region is North or West' -> filter = region IN [North, West]"""
...
def test_reg_filter_plus_grouping(sales_df):
    """Filter + Grouping: 'Calculate total sales for each product where region is North'"""
...
def test_reg_full_failing_query_end_to_end(sales_df):
    """
    Step 12 & Full Query Benchmark:
    Query:
    ...
    # 2. Group by product and region -> exactly 6 combinations
    assert res.group_by == ["product", "region"]
...
def test_reg_arbitrary_dataset_independence():
    """
